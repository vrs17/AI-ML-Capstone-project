...
- **Status:** ✅ Resolved for the capstone — labels are now human-verified golden and the model can abstain to `others`. *Production note:* at 20–50+ models a fixed softmax `others` class becomes brittle, so the scaled system will move to embedding-distance OOD (kNN / Mahalanobis) on top of the same trust layer; the `others` class remains the capstone's demonstration of open-set behavior.
